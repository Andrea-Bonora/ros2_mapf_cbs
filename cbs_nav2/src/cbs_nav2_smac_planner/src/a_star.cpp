  ...
  const std::vector<std::map<std::string, int>> ec)
{
  ...
  while (iterations < getMaxIterations() && !_queue.empty()) {
    ...
    neighbors.clear();
    // When getting the neighbors, check if they are not involved a vertex constrain (vc) or in a edge constraint (ec)
    current_node->getNeighbors(neighborGetter, _collision_checker, _traverse_unknown, neighbors, getSizeX(), vc, ec);
