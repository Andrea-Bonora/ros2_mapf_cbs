...
CBSSmacPlanner2D::CBSSmacPlanner2D()
: _a_star(nullptr),
...
{
  RCLCPP_INFO(
    _logger, "Destroying plugin %s of type CBSSmacPlanner2D",
    _name.c_str());
}

void CBSSmacPlanner2D::configure(
  ...
  std::shared_ptr<nav2_costmap_2d::Costmap2DROS> costmap_ros)
{
  ...
  RCLCPP_INFO(_logger, "Configuring %s of type CBSSmacPlanner2D", name.c_str());
  ...
  RCLCPP_INFO(
    _logger, "Configured plugin %s of type CBSSmacPlanner2D with "
...
void CBSSmacPlanner2D::activate()
{
  RCLCPP_INFO(
    _logger, "Activating plugin %s of type CBSSmacPlanner2D",
    _name.c_str());
  ...
  _dyn_params_handler = node->add_on_set_parameters_callback(
    std::bind(&CBSSmacPlanner2D::dynamicParametersCallback, this, _1));
}

void CBSSmacPlanner2D::deactivate()
{
  RCLCPP_INFO(
    _logger, "Deactivating plugin %s of type CBSSmacPlanner2D",
    _name.c_str());
...
void CBSSmacPlanner2D::cleanup()
{
  RCLCPP_INFO(
    _logger, "Cleaning up plugin %s of type CBSSmacPlanner2D",
    _name.c_str());
  _a_star.reset();
  _smoother.reset();
  if (_costmap_downsampler) {
...
nav_msgs::msg::Path CBSSmacPlanner2D::createPlan(
  const geometry_msgs::msg::PoseStamped & start,
  const geometry_msgs::msg::PoseStamped & goal,
  const std::vector<cbs_interfaces::msg::VertexConstraint> vertex_constraints,
  const std::vector<cbs_interfaces::msg::EdgeConstraint> edge_constraints,
  const bool smooth)
{
  ...
  // Set starting point
  unsigned int mx_start, cbs_start, mx_goal, cbs_goal;
  costmap->worldToMap(start.pose.position.x, start.pose.position.y, mx_start, cbs_start);
  _a_star->setStart(mx_start, cbs_start, 0);

  // Set goal point
  costmap->worldToMap(goal.pose.position.x, goal.pose.position.y, mx_goal, cbs_goal);
  _a_star->setGoal(mx_goal, cbs_goal, 0);
  ...
  if (mx_start == mx_goal && cbs_start == cbs_goal) {
    if (costmap->getCost(mx_start, cbs_start) == nav2_costmap_2d::LETHAL_OBSTACLE) {
  ...
  //Prepare two vectors: one to store the vertex constraints, the other to store the edge constraints
  //These two vectors will be used to compute a path consistent with them
...
CBSSmacPlanner2D::dynamicParametersCallback(std::vector<rclcpp::Parameter> parameters)
{
...
PLUGINLIB_EXPORT_CLASS(cbs_nav2_smac_planner::CBSSmacPlanner2D, cbs_nav2_core::MultiAgentGlobalPlanner)
