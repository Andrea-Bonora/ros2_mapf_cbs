  ...
  const double & cost_penalty)
{
  ...
  unsigned int idx, mx, my, mx_idx, cbs_idx;
  ...
  while (!obstacle_heuristic_queue.empty()) {
    ...
    for (unsigned int i = 0; i != neighborhood.size(); i++) {
      ...
      if (new_idx < size_x * size_y) {
        ...
        if (my == 0 && cbs_idx >= size_y - 1 || my >= size_y - 1 && cbs_idx == 0) {
          continue;
        }
...
void NodeHybrid::getNeighbors(
  std::function<bool(const unsigned int &, cbs_nav2_smac_planner::NodeHybrid * &)> & NeighborGetter,
  GridCollisionChecker * collision_checker,
  const bool & traverse_unknown,
