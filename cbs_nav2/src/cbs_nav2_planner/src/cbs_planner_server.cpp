...
#include "cbs_nav2_smac_planner/cbs_smac_planner_2d.hpp"
#include "nav2_costmap_2d/cost_values.hpp"
#include "cbs_nav2_planner/cbs_planner_server.hpp"

using namespace std::chrono_literals;
using rcl_interfaces::msg::ParameterType;
using std::placeholders::_1;

namespace cbs_nav2_planner
{

CBSPlannerServer::CBSPlannerServer(const rclcpp::NodeOptions & options)
: nav2_util::LifecycleNode("planner_server", "", options),
  gp_loader_("cbs_nav2_core", "cbs_nav2_core::MultiAgentGlobalPlanner"),
  default_ids_{"GridBased"},
  default_types_{"cbs_nav2_smac_planner/CBSSmacPlanner2D"},
  costmap_(nullptr)
{
...
CBSPlannerServer::~CBSPlannerServer()
{
  /*
...
CBSPlannerServer::on_configure(const rclcpp_lifecycle::State & /*state*/)
{
  ...
  for (size_t i = 0; i != planner_ids_.size(); i++) {
    try {
      planner_types_[i] = nav2_util::get_plugin_type_param(
        node, planner_ids_[i]);
      cbs_nav2_core::MultiAgentGlobalPlanner::Ptr planner =
  ...
  action_server_pose_ = std::make_unique<ActionServerToPose>(
    ...
    std::bind(&CBSPlannerServer::computePlanCBS, this),
    nullptr,
  ...
  action_server_pose_no_constraints_ = std::make_unique<ActionServerToPoseNoConstraints>(
    ...
    std::bind(&CBSPlannerServer::computePlan, this),
    nullptr,
    std::chrono::milliseconds(500),
    true);
  ...
  action_server_poses_ = std::make_unique<ActionServerThroughPoses>(
    ...
    std::bind(&CBSPlannerServer::computePlanThroughPoses, this),
    nullptr,
    std::chrono::milliseconds(500),
    true);
...
nav2_util::CallbackReturn
CBSPlannerServer::on_activate(const rclcpp_lifecycle::State & /*state*/)
{
  ...
  is_path_valid_service_ = node->create_service<nav2_msgs::srv::IsPathValid>(
    "is_path_valid",
    std::bind(
      &CBSPlannerServer::isPathValid, this,
      std::placeholders::_1, std::placeholders::_2));

  // Add callback for dynamic parameters
  dyn_params_handler_ = node->add_on_set_parameters_callback(
    std::bind(&CBSPlannerServer::dynamicParametersCallback, this, _1));
...
nav2_util::CallbackReturn
CBSPlannerServer::on_deactivate(const rclcpp_lifecycle::State & /*state*/)
{
...
nav2_util::CallbackReturn
CBSPlannerServer::on_cleanup(const rclcpp_lifecycle::State & /*state*/)
{
...
nav2_util::CallbackReturn
CBSPlannerServer::on_shutdown(const rclcpp_lifecycle::State &)
{
...
template<typename T>
bool CBSPlannerServer::isServerInactive(
...
void CBSPlannerServer::waitForCostmap()
{
...
template<typename T>
bool CBSPlannerServer::isCancelRequested(
  std::unique_ptr<nav2_util::SimpleActionServer<T>> & action_server)
{
...
template<typename T>
void CBSPlannerServer::getPreemptedGoalIfRequested(
  std::unique_ptr<nav2_util::SimpleActionServer<T>> & action_server,
...
template<typename T>
bool CBSPlannerServer::getStartPose(
  std::unique_ptr<nav2_util::SimpleActionServer<T>> & action_server,
  typename std::shared_ptr<const typename T::Goal> goal,
...
template<typename T>
bool CBSPlannerServer::transformPosesToGlobalFrame(
  std::unique_ptr<nav2_util::SimpleActionServer<T>> & action_server,
...
template<typename T>
bool CBSPlannerServer::validatePath(
  std::unique_ptr<nav2_util::SimpleActionServer<T>> & action_server,
...
void
CBSPlannerServer::computePlanThroughPoses()
{
...
void
CBSPlannerServer::computePlanCBS()
{
  std::lock_guard<std::mutex> lock(dynamic_params_lock_);
  ...
  try {
    ...
    result->path = getPlan(start, goal_pose, goal->planner_id, goal->vertex_constraints, goal->edge_constraints, goal->smooth);
...
nav_msgs::msg::Path
CBSPlannerServer::getPlan(
  const geometry_msgs::msg::PoseStamped & start,
  const geometry_msgs::msg::PoseStamped & goal,
  const std::string & planner_id,
  const std::vector<cbs_interfaces::msg::VertexConstraint> vc,
  const std::vector<cbs_interfaces::msg::EdgeConstraint> ec,
  const bool smooth
  )
{
  RCLCPP_DEBUG(
  ...
  if (planners_.find(planner_id) != planners_.end()) {
    return planners_[planner_id]->createPlan(start, goal, vc, ec, smooth);
  } else {
    if (planners_.size() == 1 && planner_id.empty()) {
      ...
      return planners_[planners_.begin()->first]->createPlan(start, goal, vc, ec, smooth);
    } else {
      RCLCPP_ERROR(
...
void
CBSPlannerServer::publishPlan(const nav_msgs::msg::Path & path)
{
...
void CBSPlannerServer::isPathValid(
...
CBSPlannerServer::dynamicParametersCallback(std::vector<rclcpp::Parameter> parameters)
{
  std::lock_guard<std::mutex> lock(dynamic_params_lock_);
...
RCLCPP_COMPONENTS_REGISTER_NODE(cbs_nav2_planner::CBSPlannerServer)
