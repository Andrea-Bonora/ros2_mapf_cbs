...
MyPlannerServer::~MyPlannerServer()
{
  /*
...
MyPlannerServer::on_configure(const rclcpp_lifecycle::State & /*state*/)
{
  ...
  for (size_t i = 0; i != planner_ids_.size(); i++) {
    try {
      planner_types_[i] = nav2_util::get_plugin_type_param(
        node, planner_ids_[i]);
      my_nav2_core::MultiAgentGlobalPlanner::Ptr planner =
...
nav2_util::CallbackReturn
MyPlannerServer::on_activate(const rclcpp_lifecycle::State & /*state*/)
{
  ...
  is_path_valid_service_ = node->create_service<nav2_msgs::srv::IsPathValid>(
    "is_path_valid",
    std::bind(
      &MyPlannerServer::isPathValid, this,
      std::placeholders::_1, std::placeholders::_2));

  // Add callback for dynamic parameters
  dyn_params_handler_ = node->add_on_set_parameters_callback(
    std::bind(&MyPlannerServer::dynamicParametersCallback, this, _1));
...
nav2_util::CallbackReturn
MyPlannerServer::on_deactivate(const rclcpp_lifecycle::State & /*state*/)
{
...
nav2_util::CallbackReturn
MyPlannerServer::on_cleanup(const rclcpp_lifecycle::State & /*state*/)
{
...
nav2_util::CallbackReturn
MyPlannerServer::on_shutdown(const rclcpp_lifecycle::State &)
{
...
template<typename T>
bool MyPlannerServer::isServerInactive(
...
void MyPlannerServer::waitForCostmap()
{
...
template<typename T>
bool MyPlannerServer::isCancelRequested(
  std::unique_ptr<nav2_util::SimpleActionServer<T>> & action_server)
{
...
template<typename T>
void MyPlannerServer::getPreemptedGoalIfRequested(
  std::unique_ptr<nav2_util::SimpleActionServer<T>> & action_server,
...
template<typename T>
bool MyPlannerServer::getStartPose(
  typename std::shared_ptr<const typename T::Goal> goal,
...
bool MyPlannerServer::transformPosesToGlobalFrame(
...
template<typename T>
bool MyPlannerServer::validatePath(
...
/*void MyPlannerServer::computePlanThroughPoses()
{
...
void MyPlannerServer::computePlan()
{
  std::lock_guard<std::mutex> lock(dynamic_params_lock_);
...
nav_msgs::msg::Path MyPlannerServer::getPlan(
...
void MyPlannerServer::publishPlan(const nav_msgs::msg::Path & path)
{
...
void MyPlannerServer::isPathValid(
...
MyPlannerServer::dynamicParametersCallback(std::vector<rclcpp::Parameter> parameters)
{
  std::lock_guard<std::mutex> lock(dynamic_params_lock_);
...
void MyPlannerServer::exceptionWarning(
  const geometry_msgs::msg::PoseStamped & start,
  const geometry_msgs::msg::PoseStamped & goal,
  const std::string & planner_id,
...
RCLCPP_COMPONENTS_REGISTER_NODE(my_nav2_planner::MyPlannerServer)
