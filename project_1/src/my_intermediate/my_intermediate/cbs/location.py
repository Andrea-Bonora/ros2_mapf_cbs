class Location(object):
    ...
    def is_conflict(self, other):
        if abs(self.pose_stamped.pose.position.x - other.pose_stamped.pose.position.x) < 0.9 and \
                    abs(self.pose_stamped.pose.position.y - other.pose_stamped.pose.position.y) < 0.9:
                return True
        return False
