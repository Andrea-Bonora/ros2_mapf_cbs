...
from cbs_nodes.cbs.conflict import Conflict
from itertools import combinations
from cbs_nodes.cbs.vertex_constraint import VertexConstraint
from cbs_nodes.cbs.edge_constraint import EdgeConstraint
        ...
        for t in range(max_t):
            for agent_1, agent_2 in combinations(solution.keys(), 2):
                ...
                if state_1.is_equal_except_time(state_2):
                    ...
                    result.time_1 = min(t, len(solution[agent_1]) - 1) 
                    result.time_2 = min(t, len(solution[agent_2]) - 1) 
            ...
            if t > 0:
                for agent_1, agent_2 in combinations(solution.keys(), 2):
                    ...
                    if state_1a.is_equal_except_time(state_2b) and state_1b.is_equal_except_time(state_2a):
                        result = Conflict()
                        result.time_1 = min(t-1, len(solution[agent_1]) - 1) 
                        result.time_2 = min(t-1, len(solution[agent_2]) - 1) 
